...
# 1 "/cvmfs/rcg.sfu.ca/sw/env/neutral/x86_64/TOOLS/XILINX/V2020.2/Vitis_HLS/2020.2/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/stddef.h" 1 3 4
# 62 "/cvmfs/rcg.sfu.ca/sw/env/neutral/x86_64/TOOLS/XILINX/V2020.2/Vitis_HLS/2020.2/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/stddef.h" 3 4
...
static void load_input_tile(float* global_ptr, float local_buff[64][64], int row_offset, int col_offset, int max_row, int max_col) {
    VITIS_LOOP_9_1: for (int i = 0; i < 64; i++) {
        VITIS_LOOP_10_2: for (int j = 0; j < 64; j++) {
...
static void compute_tile(float buff_A[64][64], float buff_B[64][64], float buff_C[64][64], float alpha) {
...
#pragma HLS ARRAY_PARTITION variable=buff_B dim=1 complete
 ...
 VITIS_LOOP_30_1: for (int i = 0; i < 64; i++) {
        VITIS_LOOP_31_2: for (int j = 0; j < 64; j++) {
#pragma HLS PIPELINE II=1

 float sum = 0.0f;

            VITIS_LOOP_36_3: for (int k = 0; k < 64; k++) {
...
static void store_output_tile(float* global_ptr, float local_buff[64][64], int row_offset, int col_offset, int max_row, int max_col) {
    VITIS_LOOP_46_1: for (int i = 0; i < 64; i++) {
        VITIS_LOOP_47_2: for (int j = 0; j < 64; j++) {
#pragma HLS PIPELINE II=1
 int r = row_offset + i;
            int c = col_offset + j;
            if (r < max_row && c < max_col)
...
#pragma HLS ARRAY_PARTITION variable=buff_B dim=1 complete
 ...
 VITIS_LOOP_75_1: for (int i = 0; i < ni; i += 64) {
        VITIS_LOOP_76_2: for (int j = 0; j < nj; j += 64) {



            VITIS_LOOP_80_3: for (int ii = 0; ii < 64; ii++) {
                VITIS_LOOP_81_4: for (int jj = 0; jj < 64; jj++) {
#pragma HLS PIPELINE II=1
 int r = i + ii;
                    int c = j + jj;
            ...
            VITIS_LOOP_91_5: for (int k = 0; k < nk; k += 64) {
