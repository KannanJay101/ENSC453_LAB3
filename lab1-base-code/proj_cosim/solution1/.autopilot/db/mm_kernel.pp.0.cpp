...
static void load_input_tile(float* global_ptr, float local_buff[16][16], int row_offset, int col_offset, int max_row, int max_col) {
...
static void compute_tile(float buff_A[16][16], float buff_B[16][16], float buff_C[16][16], float alpha) {
...
#pragma HLS ARRAY_PARTITION variable=buff_B dim=1 complete
 ...
 VITIS_LOOP_30_1: for (int i = 0; i < 16; i++) {
        VITIS_LOOP_31_2: for (int j = 0; j < 16; j++) {
#pragma HLS PIPELINE II=1

 float sum = 0.0f;

            VITIS_LOOP_36_3: for (int k = 0; k < 16; k++) {
...
static void store_output_tile(float* global_ptr, float local_buff[16][16], int row_offset, int col_offset, int max_row, int max_col) {
    VITIS_LOOP_46_1: for (int i = 0; i < 16; i++) {
        VITIS_LOOP_47_2: for (int j = 0; j < 16; j++) {
#pragma HLS PIPELINE II=1
 int r = row_offset + i;
            int c = col_offset + j;
            if (r < max_row && c < max_col)
...
#pragma HLS ARRAY_PARTITION variable=buff_B dim=1 complete
 ...
 VITIS_LOOP_75_1: for (int i = 0; i < ni; i += 16) {
        VITIS_LOOP_76_2: for (int j = 0; j < nj; j += 16) {



            VITIS_LOOP_80_3: for (int ii = 0; ii < 16; ii++) {
                VITIS_LOOP_81_4: for (int jj = 0; jj < 16; jj++) {
#pragma HLS PIPELINE II=1
 int r = i + ii;
                    int c = j + jj;
            ...
            VITIS_LOOP_91_5: for (int k = 0; k < nk; k += 16) {
